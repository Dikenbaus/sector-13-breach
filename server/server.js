...
function join(c,m){
	const r=makeRoom(String(m.room||'').toUpperCase(),m.mode);
	if(r.clients.size>=4)return send(c,{type:'error',message:'Комната заполнена'});
	c.room=r;
	c.id=uid();
	const p={id:c.id,name:String(m.name||'Ranger').slice(0,12),x:2.5,y:2.5,a:0,hp:100,score:0,dead:false};
	spawn(p);
	r.clients.set(c.id,c);
	r.players.set(c.id,p);
	send(c,{type:'welcome',id:c.id,room:r.id,mode:r.mode,player:p,timeLeft:r.timeLeft});
	broadcast(r,{type:'event',text:`${p.name} подключился`});
	// Immediately publish a fresh snapshot so all clients see each other right away.
	broadcastState(r);
}
...
function remove(c){
	...
	if(p)broadcast(r,{type:'event',text:`${p.name} покинул сектор`});
	if(!r.clients.size)rooms.delete(r.id);
