...
function message(c,m){if(m.type==='join')return join(c,m);const r=c.room,p=r&&r.players.get(c.id);if(!p)return;if(m.type==='input'&&!p.dead){const x=Number(m.x),y=Number(m.y),a=Number(m.a);if(Number.isFinite(x)&&Number.isFinite(y)&&Math.hypot(x-p.x,y-p.y)<1){p.x=Math.max(1.15,Math.min(14.85,x));p.y=Math.max(1.15,Math.min(14.85,y));p.a=a}}else if(m.type==='shoot'&&!p.dead){const now=Date.now();if(!p.shotAt||now-p.shotAt>240){p.shotAt=now;hitScan(r,p,Number(m.a)||p.a)}}else if(m.type==='respawn'){
		if(p.dead){
			spawn(p);
			broadcast(r,{type:'event',text:`${p.name} возродился`});
		} else {
			// Defensive: if client thinks they're dead but server didn't mark it,
			// still allow forcing a clean respawn to avoid desync.
			spawn(p);
		}
	}}
...
function tick(){const now=Date.now();for(const r of rooms.values()){const dt=Math.min(.1,(now-r.last)/1000);r.last=now;r.timeLeft=Math.max(0,r.timeLeft-dt);if(r.mode==='coop')for(const m of r.monsters.values()){if(m.dead)continue;let target=null,best=99;for(const p of r.players.values()){if(p.dead)continue;const d=Math.hypot(p.x-m.x,p.y-m.y);if(d<best){best=d;target=p}}if(target&&best<9){if(best>.62){const a=Math.atan2(target.y-m.y,target.x-m.x);m.x+=Math.cos(a)*dt*.52;m.y+=Math.sin(a)*dt*.52}else if((m.cool-=dt)<=0){m.cool=1;target.hp-=12;if(target.hp<=0){target.hp=0;target.dead=true;broadcast(r,{type:'event',text:`${target.name} выбыл`})}}}}broadcast(r,{type:'state',timeLeft:r.timeLeft,players:[...r.players.values()],monsters:[...r.monsters.values()]})}}
setInterval(tick,80);server.listen(PORT,()=>console.log(`Sector 13 server listening on ${PORT}`));
